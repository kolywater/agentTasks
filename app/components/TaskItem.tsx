...
export default function TaskItem({ task, view, onUpdate, onDelete, onEditStart, onEditEnd }: TaskItemProps) {
  ...
  return (
    <div className="relative overflow-hidden border-b border-gray-100">
      {/* Swipe action buttons (sit behind the row) */}
      <div className="absolute inset-y-0 right-0 flex">
        <button
          onClick={() => {
            onUpdate({ id: task.id, priority: !task.priority });
            close();
          }}
          className="w-[70px] flex items-center justify-center bg-orange-500 text-white text-xs font-semibold"
        >
          {task.priority ? "Unflag" : "Flag"}
        </button>
        <button
          onClick={() => {
            onDelete(task.id);
          ...
          className="w-[70px] flex items-center justify-center bg-red-500 text-white text-xs font-semibold"
        >
          Delete
        </button>
      </div>
      ...
      <div
        ref={rowRef}
        {...handlers}
        className={`relative z-10 flex items-start gap-3 px-4 py-3 bg-white transition-colors ${
